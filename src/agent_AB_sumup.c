...
int agent_next_move( int prev_move )
{
  ...
  for(int p = 1; p < 10; p++)
  {
    ...
    if(board[prev_move][p] == EMPTY)
    {
      ...
      int moveScore = AlphaBetaSearch(board, prev_move, p, 0, max_depth, max_depth, alpha, beta, sum);
...
*/
int AlphaBetaSearch(int myBoard[10][10], int currentBoard, int nextBoard, int myTurn, int depth, int maxDepth, int alpha, int beta, int sum)
{
  int score = intermediateScore(myBoard, currentBoard, nextBoard, !myTurn);
  double gamma = 1;
  ...
  if(myTurn == 1)
  {
      for(int p = 1; p < 10; p++)
      {
        if(myBoard[nextBoard][p] == EMPTY)
        {
          ...
          //Now official recursive step!
          alpha = max(alpha, AlphaBetaSearch(myBoard, nextBoard, p, 0, depth-1, maxDepth, alpha, beta, sum));
  ...
  else //if (myTurn == 0)
  {
    for(int p = 1; p < 10; p++)
    {
      if(myBoard[nextBoard][p] == EMPTY)
      {
        ...
        //Now official recursive step!
        beta = min(beta, AlphaBetaSearch(myBoard, nextBoard, p, 1, depth-1, maxDepth, alpha, beta, sum));
